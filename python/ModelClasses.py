    ...
    def setCorrelation(self,name1,name2,rho):
        ...
        try:
            ...
            for ix in range(len(self.parameters)):
                parameterNameList.append(self.parameters[ix]["name"])
            ...
            logging.error(e)
            if name1 not in parameterNameList:
                logging.error(name1+" not found in "+str(parameterNameList))
                sys.exit(-1)
            if name2 not in parameterNameList:
                logging.error(name2+" not found in "+str(parameterNameList))
                sys.exit(-1)
    ...
    def getParameterName(self,name):
        for parameter in self.parameters:
            if parameter["name"]==name:
                return name
        logging.error("while getting the parameter name from a multidimensional distribution")
        logging.error(name+" not in "+str(self.getParameterNames()))
        sys.exit(-1)
...
class RootProjectedHistogram(THETAObject):
    def __init__(self, name="", configDict={}):
        THETAObject.__init__(self, name)
        self._configDict={}
        self._configDict['friends']=[]
        self._configDict.update(configDict)
        
    def setFileName(self, filename):
        self._configDict['filename']=filename
        
    def setVariableString(self, var):
        self._configDict['variable']=var
        
    def setWeightString(self, weight):
        self._configDict['weight']=weight
        
    def getWeightString(self):
        return self._configDict['weight']
        
    def setTreeName(self, name):
        self._configDict['tree']=name
        
    def addFriend(self,name):
        self._configDict['friends'].append(name)
        
    def setBinning(self, nbins):
        self._configDict['nbins']=nbins
        
    def setRange(self, r):
        self._configDict['range']=r
    
    def toConfigString(self, indentLevel=0):
        retStr=self._indent(self.varname+" = {", indentLevel)
        retStr+=self._indent("type = \"root_projector\";", indentLevel+1)
        for key in self._configDict.keys():
            if key=="nbins":
...
class HistoAdd(THETAObject):
    def __init__(self, name="", configDict={}):
        THETAObject.__init__(self, name)
        self._configDict={}
        self._configDict['histoList']=[]
        self._configDict.update(configDict)
        
        
    def addHisto(self, histoname):
        self._configDict['histoList'].append(histoname)
        
    
    def toConfigString(self, indentLevel=0):
        retStr=self._indent(self.varname+" = {", indentLevel)
        retStr+=self._indent("type = \"histogram_add\";", indentLevel+1)
        for key in self._configDict.keys():
            if key=="histoList":
                srcString =""
                for srcname in self._configDict['histoList']:
                    srcString+='"@'+srcname+'",'
